...
for no, scenario in enumerate(SCENARIOS):
    scenario_elbos_list = []
    scenario_recon_loss_list = []
    scenario_kl_loss_list = []
    ...
    for pruning_iteration in range(NUM_PRUNING_CYCLES + 1):
        ...
        for epoch in range(1, EPOCH_PRUNING_CYCLE + 1):
            ...
            test_kl_loss = tf.keras.metrics.Mean()
            test_loss = tf.keras.metrics.Mean()
            for test_x in test_dataset:
                recon_loss, kl_loss = cvae.compute_loss_separately(test_x)
                test_recon_loss(recon_loss)
                test_kl_loss(kl_loss)
                test_loss(recon_loss + kl_loss)

            recon_loss_epoch = test_recon_loss.result()
            kl_loss_epoch = test_kl_loss.result()

            scenario_recon_loss_list.append(recon_loss_epoch.numpy())
            scenario_kl_loss_list.append(kl_loss_epoch.numpy())

            test_elbo = test_loss.result()
            scenario_elbos_list.append(test_elbo.numpy())

            print('Epoch: {}, Test ELBO: {}, Test Recon. L: {}, Test KL: {}, Training time: {}'
                  .format(epoch, test_elbo, recon_loss_epoch, kl_loss_epoch, end_time - start_time))

            if epoch == REWIND_WEIGHTS_EPOCH:  # and pruning_iteration == 0:  # default rewinding
    ...
    ax[0, 1].plot(np.arange(1, len(scenario_recon_loss_list) + 1), scenario_recon_loss_list)
    ax[1, 0].plot(np.arange(1, len(scenario_kl_loss_list) + 1), scenario_kl_loss_list)
...
total_epochs = EPOCH_PRUNING_CYCLE * (NUM_PRUNING_CYCLES + 1)
