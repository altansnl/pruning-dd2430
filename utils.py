...
def reset_and_load_models(latent_dim, scenario):
    ...
    test_encoder = load_model(
        f'saved_models/encoder-scenario-{scenario}.h5', compile=False)
    test_decoder = load_model(
        f'saved_models/decoder-scenario-{scenario}.h5', compile=False)
...
def calculate_time(test_dataset, latent_dim, scenario):
    ...
    decoder_inputs = test_dataset.map(
        lambda x: test_cvae.encoder(x, training=False)[:, :latent_dim])
...
def calculate_flop(test_dataset, latent_dim, scenario):
    ...
    dec_sample = tf.constant(np.expand_dims(
        test_cvae.encoder(sample)[0, :latent_dim], axis=0))
...
def calculate_params(latent_dim, scenario):
    ...
    enc_trainable_params = sum(count_params(layer)
                               for layer in test_cvae.encoder.trainable_weights)
    enc_non_trainable_params = sum(count_params(
        layer) for layer in test_cvae.encoder.non_trainable_weights)
    dec_trainable_params = sum(count_params(layer)
                               for layer in test_cvae.decoder.trainable_weights)
    dec_non_trainable_params = sum(count_params(
        layer) for layer in test_cvae.decoder.non_trainable_weights)

    total_params = enc_trainable_params + enc_non_trainable_params + \
        dec_trainable_params + dec_non_trainable_params
...
def save_and_calculate_metrics(cvae, test_dataset, scenario):
    ...
    # Time
    mean_inference_time = calculate_time(
        test_dataset, cvae.latent_dim, scenario)
