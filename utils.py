...
def calculate_metrics(cvae, test_dataset):
    ...
    enc_trainable_params = sum(count_params(layer)
                               for layer in cvae.encoder.trainable_weights)
    enc_non_trainable_params = sum(count_params(layer)
                                   for layer in cvae.encoder.non_trainable_weights)
    dec_trainable_params = sum(count_params(layer)
                               for layer in cvae.decoder.trainable_weights)
    dec_non_trainable_params = sum(count_params(layer)
                                   for layer in cvae.decoder.non_trainable_weights)
